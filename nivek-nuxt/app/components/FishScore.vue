...
const fishScores = ref<{ as_channel: FishScore[]; as_chatter: FishScore[] }>({
    as_channel: [],
    as_chatter: [],
})
...
async function getFishScore() {
    try {
        fishScores.value = await api<{ as_channel: FishScore[]; as_chatter: FishScore[] }>(
            API_ROUTES.Secure.GetFishScore
        )
    } catch (err: unknown) {
